...
node_t *rbtree_erase_find_node_min(rbtree *t, node_t *p);
...
void delete_rbtree(rbtree *t)
{
  // 노드 따라 내려갔다가 온다음 nil은 다음, 트리는 마지막에 삭제
...
{
  /*
  p는 nil까지 내려가고 temp는 nil 전에서 멈춤
  즉, new_Node는 temp의 자식이고 p(nil)의 위치에 들어가야함
  */
  node_t *temp = t->nil;
  node_t *p = t->root;
  ...
  if (temp == t->nil)  // 트리에 아무도 없었던 경우
  {
    t->root = new_Node;
  }
  else if (new_Node->key < temp->key)   // 새로운 키가 작다면 왼쪽으로 내려간다
  {
    temp->left = new_Node;
  }
  else   // 새로운 키가 크다면 오른쪽으로 내려간다
  {
    temp->right = new_Node;
  }
...
int rbtree_erase(rbtree *t, node_t *p)
{
  ...
  {
    temp = rbtree_erase_find_node_min(t, p->right);
...
void rbtree_erase_transplant(rbtree *t, node_t *p, node_t *successor)
{
  //아빠가 nil이면 트리의 root였음
  //트리의 root에 succeossor를 붙여줌
  if (p->parent == t->nil)
    t->root = successor;
...
void rbtree_erase_fixup(rbtree *t, node_t *p)
{
  ...
  {//left
    ...
    else
    {
      ...
      {
        if (sachon->left->color == RBTREE_BLACK) 
        {
          sachon->right->color = RBTREE_BLACK;
          sachon->color = RBTREE_RED;
